...
$stPA = $conn->prepare("SELECT a.*, u.name as student_name, u.email, sp.department, sp.cgpa, sp.skills, j.title as job_title, c.company_name, (SELECT score FROM resume_analysis WHERE user_id=a.student_id ORDER BY analyzed_at DESC LIMIT 1) as resume_score FROM applications a JOIN users u ON a.student_id=u.id LEFT JOIN student_profiles sp ON u.id=sp.user_id JOIN jobs j ON a.job_id=j.id JOIN companies c ON j.company_id=c.id WHERE a.status='shortlisted' AND (a.approval_status='pending' OR a.approval_status IS NULL) GROUP BY a.student_id, a.job_id ORDER BY a.applied_at DESC");
